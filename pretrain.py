    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for step, ((y1, y2), _) in enumerate(loader, start=epoch * len(loader)):
            ...
            with torch.cuda.amp.autocast():
                ...
                # barlowtwins_loss
                barlowtwins_loss = barlowtwins_loss(features[0], features[1])
            ...
            if step % args.print_freq == 0:
                if args.rank == 0:
                    ...
                    print("epoch:{}, step:{}, lr_w:{}, lr_b:{}, loss:{}, time:{}".format(
                        epoch, step, optimizer.param_groups[0]['lr'], optimizer.param_groups[1]['lr'], loss.item(),
                        int(time.time() - start_time)))
        if args.rank == 0 and epoch % args.save_freq == 0:
            # save checkpoint
            state = dict(epoch=epoch + 1, model=model.state_dict(),
                         optimizer=optimizer.state_dict())
            save_path = os.path.join(args.save_path_base, 'ckpt_' + str(epoch + 1) + '.pth')
...
class BackBoneNet(nn.Module):
    ...
    def barlowtwins_loss(self, z1, z2):
    ...
    def forward(self, x1, x2):
        ...
        barlowtwin_loss = self.barlowtwins_loss(z1, z2)
        loss = self.args.alpha * simclr_loss + self.args.beta * barlowtwin_loss
        return loss
